...
│   │   ├── Accepted.java
│   │   └── Nack.java
│   ├── events/                   # structured event emission, for viz/ replay
│   │   ├── Event.java
│   │   ├── EventLog.java
│   │   └── MessageEvents.java
...
│   │   ├── Proposer.java
│   │   └── Learner.java
│   └── viz/                      # drives named scenarios, dumps events JSON
│       └── ScenarioRunner.java
└── test/java/paxossim/
    ├── RunAllTests.java
    ├── core/
    ...
    │   └── TestRunner.java
    └── viz/
        └── ScenarioRunnerTest.java
```

## Visualization

`viz/index.html` is a minimal, static, local UI for stepping through a Paxos
run one event at a time — message arrows, per-node promised/accepted state,
the replicated log, and applied key-value state — with Play/Pause/Step/Reset
controls. No backend, no build step: open it in a browser, pick a bundled
sample from the dropdown (works fully offline), or load your own run
generated by `paxossim.viz.ScenarioRunner <scenario> <output-file>`. Its
"Build your own run" panel scripts a custom scenario (node failures,
partitions, proposer actions) and replays it instantly via a small
in-browser Paxos engine — no Java needed, good for live demos. See
...
invariant, a state machine that applies in strict slot order, network
failure injection (drop, reorder, partition) exercised end to end by
dedicated failure-scenario and cross-node safety-invariant tests, and
a static visualizer (`viz/`) for stepping through a run interactively.
121 tests pass via `RunAllTests`. See "What you would do next" above for
